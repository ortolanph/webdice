...
public class AjaxDice {
	...
	public DiceResult roll(int faces) {
		...
		try {
			Dice d = DiceFactory.createDice(faces);
			
			result.setResult(d.roll());
			result.setFaces(new int[] { faces });
			result.setPartialResults(new int[][] { d.getPartialResults() });
